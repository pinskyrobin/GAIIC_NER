...
def predict(args, model, tokenizer, prefix=""):
    ...
    results = []
    output_predict_file = os.path.join(pred_output_dir, prefix, "submit.txt")
    ...
    with open(output_predict_file, "w") as writer:
        with open(os.path.join(args.data_dir, 'test.txt'), 'r') as f:
            ...
            for line in f:
                if line == '\n':
                    submit_samples.append(submit_sample)
                    submit_sample = []
                else:
                    submit_sample.append(line.replace('\n', ''))
            if len(submit_sample) > 0:
                submit_samples.append(submit_sample)
        for sample, label in zip(submit_samples, sunmit_preds):
            for word, tag in zip(sample, label):
                writer.write(word + ' ' + args.id2label[tag] + '\n')
            writer.write('\n')

def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    if args.local_rank not in [-1, 0] and not evaluate:
        torch.distributed.barrier()  # Make sure only the first process in distributed training process the dataset, and the others will use the cache
    processor = processors[task](raw_train_path=os.path.join(args.data_dir, "train.txt"), data_dir=args.data_dir)
...
def main():
    ...
    processor = processors[args.task_name](raw_train_path=os.path.join(args.data_dir, "train.txt"), data_dir=args.data_dir)
    label_list = processor.get_labels()
